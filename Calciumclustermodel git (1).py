...
y_train2 = np.asarray(CalciumValue_list) #alternative y-value based on cut-off value of 2.0 mmol/L
...
#Calvingseason and parity are cathegorial variables, however the models functions on numerical variables only, therefore the variables are converted into binary variables using the sklearn onehotencoder. Each cathegory gets its own colummn and the column of the category that is true gets a one, all other get a zero.     
...
#all static features are added into one numpy array 
...
#again based on SampleNumber due to duplicate cows 
#convert to one line of data per cow
...
#Combine into one array for static features 
...
#flatten numpyarray to (365,418), or when upsampled to (534,418) 
...
p_x = [x_Train, x_Train_up]
p_cw = [cw1, cw2, cw3, cw4, cw5]
p_y = [y_train2, y_train2_up]
...
#loop to tune class weight and x-set
results = []
for i in range(len(p_x)):
    for j in range(len(p_cw)):
        logisticmodel = LogisticRegression(random_state=0, solver = 'liblinear', max_iter = 400, class_weight = p_cw[j]).fit(p_x[i], p_y[i])    
        pred_proba = logisticmodel.predict_proba(x_Val)
        results.append({
        'auc' : roc_auc_score(y_val2, pred_proba[:, 1]),
        'average_precision' : average_precision_score(y_val2, pred_proba[:, 1]),     
        'x' : p_x[i],
        'cw' : p_cw[j]
    })
#convert to DF
...
#evaluate model
pred_proba = logisticmodel.predict_proba(x_Test)
...
param['seed']=32 #for reproducability
...
#Sensitivity, specificity and ppv are threshold dependent, the default threshold is 0,5 
...
def DL_Model(hp):
    ...
    model.add(Dropout(rate = hp.Float('dropout', min_value=0.1, max_value = 0.8, step = 0.1)))#forget information from previous cell, prevents overfitting
    model.add(Dense(1, activation = "sigmoid")) #output must be binary, therefore sigmoid activation is necessairy   
...
#define model
model = Sequential()
model.add(LSTM(80, activation = 'relu', input_shape=(x_train.shape[1], x_train.shape[2]), return_sequences=False)) #left relu activation out since tanh en sigmoid already cause non linearity
...
history = model.fit(x_train, y_train, epochs=100, batch_size = 42, validation_data=(x_val, y_val), class_weight = {0:1, 1:3}, callbacks = [my_callbacks]) 
...
# DBTITLE 1,non upsampled, y-set 1 
...
#For every combination of y and x sets hyperparametertuning was performed seperatly and each 200 times 
import random
...
# DBTITLE 1,non upsampled, y-set 2
# A functional model was used for hyperparameter tuning, works at the same way as a sequential model but is compatinble with multi-input and therefore needed when adding static features.  
import random
#define inputs, in this case single input model
sensor_input = keras.Input(shape=(22,19))
...
#define MLP-layer  
MLP_output = Dense(30, activation = 'relu')(all_features)

#add dropout 
Dropout_output = Dropout(rate = 0.1)(MLP_output)
...
history = Model.fit([x_train, x_static_lean], y_train2, batch_size = 22, epochs=100, class_weight = {0:1, 1:3}, validation_data=([x_val, x_static_val_lean], y_val2), callbacks = [my_callbacks], use_multiprocessing = True )
...
# DBTITLE 1,func, static 8, non up, y-set 1
...
for i in range(100):
    ...
    Model.fit([x_train, x_static_lean], y_train, batch_size = BS, epochs=100, class_weight = Class_weights, validation_data=([x_val,x_static_val_lean], y_val), callbacks = [my_callbacks],  use_multiprocessing = True,  verbose = 0)
...
# DBTITLE 1,func, static 8, up, y-set 1
#hyperparameter tuning using random search for multi-input functional model
from keras.layers import Dense, LSTM, Dropout, BatchNormalization
import random
#define inputs
...
for i in range(200):
    ...
    Model.compile(optimizer = 'Adam', loss='binary_crossentropy',  metrics = ['AUC', 'Precision', 'Recall'])
    #fit model, add earlystopping and validation data 
    Model.fit([x_train_up, x_static_up_lean], y_train_up, batch_size = BS, epochs=100, class_weight = Class_weights, validation_data=([x_val,x_static_val_lean], y_val), callbacks = [my_callbacks],  use_multiprocessing = True,  verbose = 0)
    #predict y-values validation set
    pred = Model.predict([x_val, x_static_val_lean])
    pred_clases = np.where(np.squeeze(pred) < 0.5, 0, 1) 
    #append model results to list and the hyperparameters used to make the model 
...
# DBTITLE 1,func, static 8, non up, y-set 2
#hyperparameter tuning using random search for multi-input functional model
from keras.layers import Dense, LSTM, Dropout, BatchNormalization
import random
#define inputs
...
# DBTITLE 1,func, static 8, up, y-set 2
#hyperparameter tuning using random search for multi-input functional model
from keras.layers import Dense, LSTM, Dropout, BatchNormalization
import random
#define inputs
...
# DBTITLE 1,func, static 10, non up, y-set 1
#hyperparameter tuning using random search for multi-input functional model
from keras.layers import Dense, LSTM, Dropout, BatchNormalization
import random
#define inputs
...
# DBTITLE 1,func, static 10, non up, y-set 2
#hyperparameter tuning using random search for multi-input functional model
from keras.layers import Dense, LSTM, Dropout, BatchNormalization
import random
#define inputs
...
# DBTITLE 1,func, static 10, up, y-set 1
#hyperparameter tuning using random search for multi-input functional model
from keras.layers import Dense, LSTM, Dropout, BatchNormalization
import random
#define inputs
...
# DBTITLE 1,func, static 10, up, y-set 2
#hyperparameter tuning using random search for multi-input functional model
from keras.layers import Dense, LSTM, Dropout, BatchNormalization
import random
#define inputs
